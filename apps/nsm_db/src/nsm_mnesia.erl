...
list_membership(UId) when is_list(UId) -> nsm_db:select(group_member, UId).
list_membership_count(#user{username = UId}) -> list_membership_count(UId);
list_membership_count(UId) when is_list(UId) -> [{G, length(list_group_users(element(3, G)))} || G <-nsm_db:select(group_member, UId)].
list_group_users(GId) -> nsm_db:select(group_member_rev, GId).
membership(UserId, GroupId) -> just_one(fun() -> mnesia:match_object(#group_member{id = {UserId, GroupId}, _='_'}) end).
get_save_tables(UId) -> nsm_db:select(save_game_table,UId).
...
subscribe_user(MeId, FrId) -> nsm_db:put([#subscription{who = MeId, whom = FrId},
                              #subscription_rev{whom = FrId, who = MeId}]).

remove_subscription(MeId, FrId) -> nsm_db:delete([#subscription{who = MeId, whom = FrId},
                                   #subscription_rev{whom = FrId, who = MeId}]).
