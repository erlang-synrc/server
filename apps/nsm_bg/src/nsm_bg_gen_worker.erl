...
%% @doc
%%   Generalized bg worker behaviour
%% @end
%%-------------------------------------------------------------------
-module(nsm_bg_gen_worker).
...
-export([start_link/2,
         ...
         delivery/2]).  %% callback for message queue delivery

%% gen_server callbacks
-export([init/1, handle_call/3, handle_cast/2, handle_info/2,
         terminate/2, code_change/3]).

%% Behabiour callbacks list
...
delivery(Envelope, Server) ->
    gen_server:cast(Server, {delivery, Envelope}).
...
init([Module | Args]) ->
    ...
    case catch Module:init(Args) of
        {ok, CState} ->
            ...
            %% subscribe for client's events
            Options = Module:get_opts(CState),
            {ok, CallbackQueue} =
                subscribe_client(Channel, Options),

            %% subscribe for system events
            {ok, SystemQueue} = subscribe_system(Channel),
        ...
        {'EXIT', Reason} ->
            {stop, {init_failed, {Module, Reason}}}
    end.
...
handle_cast({delivery, Envelope}, State) ->
    ...
    Route   = nsm_mq_lib:key_to_list(Envelope#envelope.routing_key),

    process_delivery(Route, Envelope, State).

handle_info({'DOWN', MRef, process, Ch, Reason},
            #state{channel = Ch, channel_mon = MRef,
                   callback_state = CS} = State) ->
    ?ERROR("channel failed: ~p. CallbackState: ~p. Restart.", [Reason, CS]),
    {stop, {channel_failed, Ch}, State};

handle_info(Info, #state{callback = Module,
                         callback_state = CState0} = State)->
...
process_delivery(["notice", "system", "reconfigure"],
                 #envelope{payload = Conf}, State) ->

    ?INFO("recofigure received: ~p", [Conf]),
    {noreply, State};
process_delivery(Route, Envelope, #state{callback = Module,
                                         callback_state = CState0} = State) ->
    ?DBG("process delivery. Not system call: ~p", [Route]),

    Payload = Envelope#envelope.payload,

    %% call callbak module to process notification
    process_result(State, catch Module:handle_notice(Route, Payload, CState0)).


%% @doc process handle_notice and handle_info result
...
process_result(State, Result) ->
    ...
    case Result of
        ...
        {send, SRoutingKey, SMessage, CState1} ->
            publish(Channel, ?NOTIFICATIONS_EX, SRoutingKey, SMessage),
            {noreply, State#state{callback_state = CState1}};

        {send, SExchange, SRoutingKey, SMessage, CState1} ->
            publish(Channel, SExchange, SRoutingKey, SMessage),
            {noreply, State#state{callback_state = CState1}};

        {stop, Reason, CState1} ->
            {stop, Reason, State#state{callback_state = CState1}};
        Unexpected ->
            {stop, Unexpected, State}
    end.

%% @doc Read routing keys from db and subscribe

subscribe_client(Channel, SubsOptions) ->

    Routes    = ?gv(routes, SubsOptions),
    Queue     = ?gv(queue,  SubsOptions),
    Exchange  = ?gv(exchange,  SubsOptions, ?NOTIFICATIONS_EX),
    QueueOpts = ?gv(queue_options, SubsOptions,
                    ?DEFAULT_WORKER_QUEUE_OPTIONS),
    ConsumeOpts = ?gv(consume_options, SubsOptions,
                      ?DEFAULT_WORKER_CONS_OPTIONS),
...
subscribe_system(Channel) ->
    Routes    = ?gv(routes, ?SYSTEM_SUB_OPTS),
    Queue     = ?gv(queue,  ?SYSTEM_SUB_OPTS),
    QueueOpts = ?gv(queue_options, ?SYSTEM_SUB_OPTS),
...
subscribe(Channel, Exchange, Routes, Queue, QueueOptions, ConsumeOptions) ->
    ?INFO("gen_worker: ~p, Exchange: ~p, Routes: ~p, Queue:~p, QueueOptions: ~p",
              [Channel, Exchange, Routes, Queue, QueueOptions]),

    {ok, _} = nsm_mq_channel:create_queue(Channel, Queue, QueueOptions),

    [begin
         RoutingKey = nsm_mq_lib:list_to_key(Route),
         ok = nsm_mq_channel:bind_queue(Channel, Queue, Exchange, RoutingKey)
     end || Route <- Routes],

    ConsumeOpts = [{callback, {?MODULE, delivery}},
                   {state, self()}|ConsumeOptions],
...
publish(Channel, Exchange, RoutingKey, Message) ->
    BinaryRoutingKey =
        case RoutingKey of
            L when is_list(L)->
                nsm_mq_lib:list_to_key(L);
            B when is_binary(B) ->
                B;
            _ ->
                throw({unexpected_routing_key_format, RoutingKey})
        end,
