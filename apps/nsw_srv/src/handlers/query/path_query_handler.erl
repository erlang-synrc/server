...
%% @doc Query handler to convert path to part of the query string.
%% /modulename/key1/value1/key2/value2 -> [{key1, "value1"}, {key2, "value2"}]
%% Also, "-" in path parameters names replaced with "_". For example:
%% if path is /buy/new-tab/paypal/, then you can acess to "new-tab" parameter
%% by calling wf:q(new_tab).
...
path_params([Key, Value | Rest]) ->
    [{strip_dash(Key), mochiweb_util:unquote(Value)}|path_params(Rest)].
...
test()->
    ?assertMatch([{"__submodule__", "sub"}, {"a", "a1"}], get_params_from_path("sub/a/a1", undefined)),
    ?assertMatch([{"__submodule__", "sub"}, {"a", "a1"}], get_params_from_path("sub/a/a1", undefined)),
    ?assertMatch([{"a", "a1"}, {"b", "b1"}], get_params_from_path("a/a1/b/b1", undefined)),
    ?assertMatch([{"x", "a1"}, {"y", "b1"}, {"c", true}], get_params_from_path("a1/b1/c", ["x", "y"])),
    ?assertMatch([{"x", "a1"}, {"y", "b1"}, {"c", true}], get_params_from_path("a1/b1/c", ["x", "y"])),
    ?assertMatch([{"x", "a1"}], get_params_from_path("a1", ["x", "y"])),
    ?assertMatch([], get_params_from_path("/", ["x", "y"])),
    ?assertMatch([], get_params_from_path("/", undefined)).
