...
body_authorized() ->
    ...
    Links = [
	     {users,		?_U("/admin"),			?_T("Users")},
	     {games,		?_U("/admin/games"),		?_T("Games")},
	     {tournaments,	?_U("/admin/tournaments"),	?_T("Tournaments")},
	     {system,		?_U("/admin/system"),		?_T("System Parameters")},
	     {stats,		?_U("/admin/stats"),		?_T("Statistics")},
	     {others,		?_U("/admin/others"),		?_T("Others")},
	     {reports,		?_U("/admin/reports"),		?_T("Reports")},
	     {affiliates,		?_U("/admin/affiliates"),   ?_T("Affiliates")}
	    ],
...
get_tree(Branch) ->
    FullTree = rpc:call(?APPSERVER_NODE,zealot_db,all,[config]),
...
config_edit_box(Key) ->
	...
	case Key of
		...
		KS = wf:f("~w", [Key]),
		NotSet = make_ref(),
		{ok, Val} = rpc:call(?APPSERVER_NODE,zealot_db,get,[config, Key, NotSet]),
		case Val of
		    NotSet ->
			{KS, "", string};
...
users_list() ->
    FirstCursor = cursor:new(rpc:call(?APPSERVER_NODE, zealot_db, all, [user]), 2),
...
users_list_get_data(UsersList) ->
    [ begin
        ...
        LastLogin =
        case rpc:call(?APPSERVER_NODE,users,user_status,[Username]) of
            {error, _} ->
                "-";
...
real_affiliates_list() ->
    rpc:call(?APPSERVER_NODE,nsm_affiliates2,affiliates,[]).

is_able_to_look_details(User) ->
    rpc:call(?APPSERVER_NODE,nsm_affiliates2,is_able_to_look_details,[User]).
...
affiliates_contracts_body() ->    
    ...
    TypeList = rpc:call(?APPSERVER_NODE,nsm_affiliates2,get_contract_types,[]),
    [
...
create_new_contract_from_form() ->
    ...
    Res = rpc:call(?APPSERVER_NODE,nsm_affiliates2,create_contract,[
        UserId, Name, 
...
event(add_new_contract) ->
    UserId = wf:q(contract_new_user),
    case rpc:call(?APPSERVER_NODE, users, get_user, [{username, UserId}]) of
        {ok, _} ->
            case rpc:call(?APPSERVER_NODE, nsm_affiliates2, is_existing_affiliate, [UserId]) of
                true ->
                    create_new_contract_from_form();
                false ->
...
event({make_affiliate_and_add_contract, UserId}) ->
    case rpc:call(?APPSERVER_NODE, users, get_user, [{username, UserId}]) of
        {ok, _} ->
            rpc:call(?APPSERVER_NODE,nsm_affiliates2,create_affiliate,[UserId]);
        _ -> 
            wf:wire(#alert{text=?_TS("User '$username$' does not exist!", [{username, UserId}]) })
    end,
    create_new_contract_from_form();
...
event(add_new_contract_type) ->
    ...
    rpc:call(?APPSERVER_NODE, nsm_affiliates2, create_contract_type, [Name, Duration, Limit, Commission]),
    wf:replace(affiliates_contracts, affiliates_contracts_body());

event(disable_old_contract_type) ->
    Id = wf:q(old_contract_type),
    rpc:call(?APPSERVER_NODE, nsm_affiliates2, disable_contract_type, [Id]),
    wf:replace(affiliates_contracts, affiliates_contracts_body());
...
event(add_affiliate) ->
    ...
    case rpc:call(?APPSERVER_NODE, users, get_user, [{username, AffiliateUsername}]) of
        {ok, _} ->
            rpc:call(?APPSERVER_NODE,nsm_affiliates2,create_affiliate,[AffiliateUsername]),
...
event(remove_affiliate) ->
    ...
    case lists:member(AffiliateUsername, real_affiliates_list()) of
        true ->
            rpc:call(?APPSERVER_NODE,nsm_affiliates2,delete_affiliate,[AffiliateUsername]),
            wf:update(affiliates_list, affiliates_list());
        false ->
...
event(allow_details_affiliate) ->
    ...
    case lists:member(AffiliateUsername, real_affiliates_list()) of
        true ->
            rpc:call(?APPSERVER_NODE,nsm_affiliates2,enable_to_look_details,[AffiliateUsername]),
            wf:update(affiliates_list, affiliates_list());
        false ->
            wf:wire(#alert{text=?_TS("User '$username$' is not an affiliate!", [{username, AffiliateUsername}]) })
    end;
...
event(disallow_details_affiliate) ->
    ...
    case lists:member(AffiliateUsername, real_affiliates_list()) of
        true ->
            rpc:call(?APPSERVER_NODE,nsm_affiliates2,disable_to_look_details,[AffiliateUsername]),
            wf:update(affiliates_list, affiliates_list());
        false ->
            wf:wire(#alert{text=?_TS("User '$username$' is not an affiliate!", [{username, AffiliateUsername}]) })
    end;
    ...
    {Deleted, NotExpired} = lists:foldl(
        fun(#invite_code{created_user = CUser, create_date = CData0, code = Code} = I, {Counter, Acc}) ->
            ...
            case {CUser, Expired > 0} of
                {undefined, true} ->
                    rpc:call(?APPSERVER_NODE, zealot_db, delete, [invite_code, Code]),
                    {Counter + 1, Acc};
                _ ->
                    {Counter, [I|Acc]}
            end
...
u_event(config_save_new) ->
	...
	{ok, Key} ->
	    ...
	    case Value of
		{ok, NewValue} ->
		    rpc:call(?APPSERVER_NODE,zealot_db,put,[#config{key = Key,value=NewValue}]),
